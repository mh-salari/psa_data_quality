...
# Calculate relative increase in RMS-S2S for constricted vs dilated conditions
relative_change <- aggregated_data %>%
  ...
  mutate(percent_change = 100 * (constricted - dilated) / dilated) %>%
...
for (tracker in trackers) {
  ...
  if (nrow(tracker_change) > 0) {
    cat("  Percent Change (constricted vs dilated):\n")
...
for (tracker in unique(aggregated_data$eye_tracker)) {
  ...
  if (nrow(paired_data) >= 3) {  # Enough paired samples for a paired test
    ...
    dilated_data <- tracker_data$rms_s2s[tracker_data$trial_condition == "dilated"]
    constricted_data <- tracker_data$rms_s2s[tracker_data$trial_condition == "constricted"]
...
cat("\n===============================================================\n")
cat("Statistical Analysis of RMS-S2S Differences (dilated vs. constricted)\n")
cat("===============================================================\n")
...
dilated_overall <- aggregated_data$rms_s2s[aggregated_data$trial_condition == "dilated"]
constricted_overall <- aggregated_data$rms_s2s[aggregated_data$trial_condition == "constricted"]
