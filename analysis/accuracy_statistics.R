...
basic_stats <- accuracy_df %>%
  ...
  summarize(
    ...
    q25 = quantile(accuracy, 0.25, na.rm = TRUE),
    q75 = quantile(accuracy, 0.75, na.rm = TRUE),
    .groups = "drop"
  )

# Print basic statistics in a well-formatted manner
cat("\n================================================================\n")
cat("Basic Descriptive Statistics of Accuracy by Eye Tracker and Condition\n")
cat("================================================================\n\n")
...
for (tracker in trackers) {
  ...
  for (i in 1:nrow(tracker_data)) {
    ...
    cat("  Mean error:", round(tracker_data$mean[i], 3), "degrees (SD =", round(tracker_data$sd[i], 3), ")\n")
    cat("  Median error:", round(tracker_data$median[i], 3), "degrees\n")
...
write.csv(stat_results, "./output/accuracy_statistical_analysis.csv", row.names = FALSE)

# Save basic statistics to CSV in the current working directory
write.csv(basic_stats, file = "./output/accuracy_descriptive_stats.csv", row.names = FALSE)

# Print a message confirming where files were saved
cat("\nFiles saved to current working directory:", getwd(), "\n")
cat("Saved files:\n")
cat("- ./output/accuracy_statistical_analysis.csv\n")
cat("- ./output/accuracy_descriptive_stats.csv\n")